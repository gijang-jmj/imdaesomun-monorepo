...
const showMoreFilesText = computed(() => {
  return showAllFiles.value
    ? '첨부파일 접기'
    : `첨부파일 더보기(${hiddenFilesCount.value})`;
});
...
<template>
  <div
    class="mx-4 mt-2 mb-24 rounded-xl bg-white p-4 shadow-sm transition-colors md:p-6"
  >
    ...
    <div
      class="mb-4 flex flex-col items-start gap-2 border-b border-gray-100 pb-4"
    >
      <p class="text-label-bold text-gray-500">
        첨부파일 <span class="text-caption">* 미리보기만 제공해요</span>
      </p>
      <span v-if="notice.files.length === 0" class="text-body text-gray-500"
        >첨부파일 없음</span
      >
      <div v-for="(file, index) in visibleFiles" :key="index">
        <button
          ...
          class="text-body cursor-pointer text-start text-teal-500 underline"
        >
          {{ file.fileName }}
        </button>
        <a
      ...
      <button
        ...
        class="text-label cursor-pointer text-teal-500"
      >
        {{ showMoreFilesText }}
      </button>
    </div>
    <div class="text-body mb-8 flex flex-col gap-4 break-all">
      <div
        class="flex flex-col gap-4"
        v-for="(content, index) in notice.contents"
        :key="index"
      >
        <p
          v-for="(line, lineIndex) in splitByNewline(content)"
          :key="lineIndex"
        >
          {{ line }}
        </p>
    ...
    <div class="flex gap-2">
      ...
      >
        <span class="text-body hidden md:inline-block">{{
          saveButtonText
        }}</span>
